...
2) Обработка секции с данными, размещение их в памяти
3) Обработка секции с кодом, создание списка инструкций. Создание списка меток и их адресов
4) Расположение адресов меток в памяти данных
5) Замена в инструкциях названий меток на их реальные адреса
6) Замена в инструкциях `JMP, JZ, JNZ, JGE, LD, ST` относительной адресации на абсолютную. Замена команды `mov` на команду `ld`.
